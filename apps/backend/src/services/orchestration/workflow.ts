...
export class AgenticWorkflowOrchestrator {
  async runWorkflow(context: AgentContext) {
    ...
    loggingAgent.log('Orchestrator', 'Initialization', 'Agentic Workflow Started for incoming emergency signal.');
    ...
    try {
      ...
      if (audioResult.confidence === 'Low') {
        loggingAgent.log('Orchestrator', 'Termination', 'Audio confidence is Low. Filtering out as noise/casual conversation.');
