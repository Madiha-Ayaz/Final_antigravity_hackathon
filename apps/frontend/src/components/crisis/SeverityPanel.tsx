...
const SeverityPanel: React.FC<SeverityProps> = ({ level, confidence, radius, duration }) => {
  const getColors = () => {
    switch (level) {
      case 'Critical': return 'from-red-600 to-rose-600 border-red-500/50 text-red-50';
      case 'High': return 'from-orange-600 to-amber-600 border-orange-500/50 text-orange-50';
      case 'Medium': return 'from-indigo-600 to-blue-600 border-indigo-500/50 text-indigo-50';
      default: return 'from-emerald-600 to-teal-600 border-emerald-500/50 text-emerald-50';
    }
  };
  ...
  return (
    <div className={`rounded-2xl p-6 border bg-gradient-to-br ${getColors()} shadow-2xl`}>
      <div className="flex justify-between items-start mb-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest opacity-70 mb-1">Current Status</p>
