      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              href="/contacts"
              className="px-4 py-2 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-xl hover:from-red-500 hover:to-orange-500 transition-all text-sm font-medium flex items-center gap-2 shadow-lg shadow-red-500/20"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
              ...
              <div key={index} className={`card p-4 ${stat.href ? 'cursor-pointer' : ''}`}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-2xl sm:text-3xl font-black" style={{ color: '#f1f5f9' }}>
                      {stat.value}
                    </p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2 card p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-white">Live Location Tracking</h2>
                <p className="text-xs sm:text-sm text-slate-400">Real-time GPS synchronization</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <div
                  className={`px-3 py-1 rounded-full text-xs font-bold ${isMonitoring ? 'badge-success' : 'badge-danger'}`}
                >
                ...
                <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-10 rounded-xl">
                  <div className="text-center p-6 bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 max-w-xs">
                    ...
                    <p className="text-sm text-slate-400 mb-4">
                      Emergency detection is currently disabled.
                    </p>
                    <button
                      onClick={() => setIsMonitoring(true)}
                      className="w-full px-4 py-2 bg-gradient-to-r from-red-600 to-orange-600 text-white rounded-xl font-bold"
                    >
                      Resume Protection
                    </button>
                  </div>
          ...
          <div className="card p-4 sm:p-6 space-y-4">
            ...
            {contacts.length === 0 ? (
              ...
              <div className="space-y-2 stagger">
                ...
                <Link
                  href="/contacts"
                  className="block text-center py-2 text-sm hover:opacity-80 transition-opacity"
                  style={{ color: '#dc2626' }}
                >
                  + Add More Contacts
                </Link>
          ...
          <div className="space-y-2 stagger">
            {recentActivity.map((activity, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-sm" style={{ color: '#f1f5f9' }}>
                    {activity.title}
                  </h3>
